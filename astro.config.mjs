...
function defaultLayoutPlugin() {
  return function (tree, file) {
    ...
    // title will be the first heading element that appears in the article
    for (let child of tree.children) {
      if (child.type == "heading") {
        file.data.astro.frontmatter.title = child?.children[0]?.value;
        break;
      }
    }
    ...
    if (tree.children[1]?.value) {
      const imageElement = parse(tree.children[1].value).querySelector("img");
      file.data.astro.frontmatter.pic = imageElement.getAttribute("src");
    }

    // 描述放到文档中头图的下一行，会自动帮你处理，也可以用 frontmatter 方式，赋值给 desc 字段
    if (tree.children[2]?.children[1]?.value) {
      file.data.astro.frontmatter.desc = tree.children[2].children[1].value;
    }

    const { date, desc, pic, title } = file.data.astro.frontmatter;
    ...
    if (!title) {
      file.data.astro.frontmatter.title = SITE.title;
    }
